...
function WritWorthy.MatUI:LazyInit()
  ...
  local cb_name = nil
  local cb = WINDOW_MANAGER:CreateControlFromVirtual(cb_name, container, "ZO_ComboBox")
  ...
  local initial_filter = WritWorthy.savedVariables.mat_ui.filter_name or WritWorthy.MatUI.FILTER_NAME_ALL_MATS
...
function WritWorthy.MatUI.OnResizeStop()
  ...
  Util.OnResizeStop(WritWorthyMatWindow, WritWorthy.MatUI, WritWorthy.MatUI.scroll_filter_list, WINDOW_POS_KEY)
end
...
function WritWorthy.MatUI.HeaderInit(control, name, text, key)
  Log.Debug("WWMUI_HeaderInit() c:%s n:%s t:%s k:%s", tostring(control), name, text, key)
...
function WritWorthy.MatUI:CreateRowControlCells(row_control, header_container)
  for i, cell_name in ipairs(self.CELL_NAME_LIST) do
    ...
    Util.SetCellToHeaderAlign(cell_control, header_container, self.list_header_controls[cell_name])
...
local function abbr_num(num)
  return ZO_AbbreviateNumber(num, NUMBER_ABBREVIATION_PRECISION_LARGEST_UNIT, true)
end
...
function WritWorthy.MatUI:FilterScrollList()
  ...
  for _, mat_row_data in ipairs(self.mat_row_data_list) do
    table.insert(scroll_data, ZO_ScrollList_CreateDataEntry(DATA_TYPE_ID, mat_row_data))
  end
end
...
function WritWorthy.MatUI:SortScrollList()
  ...
  Log.Debug(
    string.format(
      "WWMUI:SortScrollList() key=%s order=%s",
  ...
  local sort_function = function(row_a, row_b)
    ...
    return ZO_TableOrderingFunction(row_a.data, row_b.data, key, WritWorthy.MatUI.SORT_KEYS, order)
  end
  local scroll_data = ZO_ScrollList_GetDataList(self.list)
...
function WritWorthy.MatUI.Cell_OnMouseDown(cell_control)
  ...
  if
  not (cell_control and cell_control.mat_row_data and cell_control.mat_row_data.mat_row and
    cell_control.mat_row_data.mat_row.link)
  then
    return
  end
