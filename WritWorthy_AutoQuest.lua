...
function WritWorthy:AQAddKeyBind()
  ...
  if not WritWorthy.RequireLibCraftText() then
    return
  end
...
function WritWorthy:StartAutoAcceptMode()
  ...
  EVENT_MANAGER:RegisterForEvent(name, EVENT_QUEST_ADDED, WritWorthy_AutoAcceptModeQuestAdded)

  EVENT_MANAGER:RegisterForEvent(name, EVENT_QUEST_OFFERED, WritWorthy_AutoAcceptModeQuestOffered)
...
function WritWorthy:EndAutoAcceptMode()
  ...
  for _, event_id in ipairs(event_list) do
    EVENT_MANAGER:UnregisterForEvent(name, EVENT_QUEST_ADDED)
  end
  ResetChatter()
end
...
function WritWorthy:RegisterRolisChatter()
  ...
  EVENT_MANAGER:RegisterForEvent(
    ...
    EVENT_CHATTER_BEGIN,
    function()
      WritWorthy:OnRolisChatterBegin()
    end
  )
  EVENT_MANAGER:RegisterForEvent(
    name,
    ...
    function(event_id, quest_index)
      WritWorthy:OnRolisQuestCompleteDialog(event_id, quest_index)
    end
  )
end
...
function WritWorthy:UnregisterRolisChatter()
  ...
  for _, event_id in ipairs(event_list) do
    EVENT_MANAGER:UnregisterForEvent(name, event_id)
  end
end
...
function WritWorthy:OnRolisQuestCompleteDialog(event_id, quest_index)
  ...
  zo_callLater(
    function()
      WritWorthy:RolisCompleteQuest(quest_index)
    end,
    500
  )
end
  ...
  zo_callLater(
    function()
      for _, event_id in ipairs(self.event_list) do
        EVENT_MANAGER:RegisterForEvent(
          WritWorthy.name .. "_aq_" .. self.name,
          event_id,
          function()
            self:Invalidate()
          end
...
function WritWorthy.AQCache:Unregister()
  ...
  for _, event_id in ipairs(self.event_list) do
    EVENT_MANAGER:UnregisterForEvent(WritWorthy.name .. "_aq_" .. self.name, event_id)
  end
end
