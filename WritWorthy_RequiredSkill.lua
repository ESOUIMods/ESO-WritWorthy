...
function RequiredSkill:ToKnow()
  ...
  if self.function_name == "IsMaxxed" then
    ...
    local text = string.format(WW.Str("know_err_skill_not_maxed"), self:Name(), self._have or -1, self._max or -1)
...
function RequiredSkill:FetchInfo()
  if self._name and (self._is_purchased ~= nil) then
    return
  end
  ...
  if not (skill_index and ability_index) then
    return
  end
  local info = {
    GetSkillAbilityInfo(SKILL_TYPE_TRADESKILL, skill_index, ability_index)
  }
  if not (info and info[1]) then
    return
  end
  self._name = info[1]
...
function RequiredSkill:FetchUpgradeInfo()
  if (self._have ~= nil and self._max ~= nil) then
    return
  end
  -- Unable to fetch info about this skill for some
  -- reason? Set fallbacks to ignore this requirement.
  ...
  local have, max = GetSkillAbilityUpgradeInfo(SKILL_TYPE_TRADESKILL, skill_index, ability_index)
  if not (have and max) then
    return
  end
  self._have = have
  self._max = max
...
function RequiredSkill.FindAllSkills()
  ...
  for skill_index = 1, num_lines do
    ...
    Log:Add(
      "t:" ..
        tostring(skill_type) .. " i:" .. tostring(skill_index) .. "  num_abilities:" .. tostring(num_abilities)
    )
