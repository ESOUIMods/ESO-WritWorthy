...
function WritWorthy.InitAGSIntegration(trading_house_wrapper)
  ...
  if WritWorthy.ags_init_started then
    return
  end
  WritWorthy.ags_init_started = true
  if not (AGS and AGS.GetAPIVersion and AGS.GetAPIVersion() == 4) then
    return
  end
  ...
  AGS:RegisterFilterFragment(AGS.class.PriceRangeFilterFragment:New(FILTER_ID.WRIT_WORTHY_WRIT_COST_FILTER))
end
...
function WritWorthy.RegisterAGSInitCallback()
  local AGS = AwesomeGuildStore -- for less typing
  if WritWorthy.ags_callback_registered then
    return
  end
  ...
  AGS:RegisterCallback(AGS.callback.AFTER_FILTER_SETUP, WritWorthy.InitAGSIntegration)
end
