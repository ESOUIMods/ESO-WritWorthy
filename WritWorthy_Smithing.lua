...
function Parser:ParseItemLink(item_link)
  ...
  Log:Add("request_item", tostring(item_num) .. " " .. tostring(self.request_item.item_name))
  self.crafting_type = self.request_item.school.trade_skill_type
  self.set_bonus = self:GetSetBonus(set_num)
  if not self.set_bonus then
  ...
  self.trait.trait_name = self.trait.trait_name or WritWorthy.SI("SI_ITEMTRAITTYPE" .. tostring(trait_num))

  self.trait.mat_link = self.trait.mat_link or GetSmithingTraitItemLink(trait_num + 1)
...
function Parser:ToMatList()
  ...
  local ml = {}
  table.insert(ml, MatRow:FromName(self.request_item.school.base_mat_name, self.request_item.base_mat_ct))
  ...
  if 0 < self.improve_level.green_mat_ct then
    table.insert(ml, MatRow:FromName(self.request_item.school.green_mat_name, self.improve_level.green_mat_ct))
  end
  if 0 < self.improve_level.blue_mat_ct then
    table.insert(ml, MatRow:FromName(self.request_item.school.blue_mat_name, self.improve_level.blue_mat_ct))
  end
  if 0 < self.improve_level.purple_mat_ct then
    table.insert(ml, MatRow:FromName(self.request_item.school.purple_mat_name, self.improve_level.purple_mat_ct))
  end
  if 0 < self.improve_level.gold_mat_ct then
    table.insert(ml, MatRow:FromName(self.request_item.school.gold_mat_name, self.improve_level.gold_mat_ct))
  end
  self.mat_list = ml
...
function Parser:ToKnowList()
  ...
  if self.request_item.school.motif_required then
    ...
    local rr = LCK.GetMotifKnowledgeForCharacter(self.motif_num, self.request_item.motif_page)
  ...
  local line_name = GetSmithingResearchLineInfo(self.request_item.school.trade_skill_type, self.request_item.research_line)
  line_name = line_name:lower()
  Log:Add(
    "GetSmithingResearchLineInfo(" ..
      "skill=" ..
      tostring(self.request_item.school.trade_skill_type) ..
      ", line=" .. tostring(self.request_item.research_line) .. ") = " .. tostring(line_name)
  )
  ...
  Log:Add(
    "GetSmithingResearchLineTraitInfo(" ..
      ...
      tostring(self.request_item.school.trade_skill_type) ..
      ", line=" ..
      tostring(self.request_item.research_line) ..
      ", trait=" .. tostring(self.trait.trait_index) .. ") = " .. tostring(trait_known)
  )
  ...
  if self.set_bonus and self.set_bonus.trait_ct then
    ...
    local msg = string.format(
      WritWorthy.Str("know_err_trait_ct_too_low"),
      known_trait_ct,
    ...
    if llc and self.set_bonus.set_id then
      ...
      local llc_can_craft = self.set_bonus.set_id and t and (t[self.set_bonus.set_id] ~= nil)
      local msg = string.format(
        WritWorthy.Str("know_err_llc_too_old"),
...
function Parser:WarningText()
  ...
  if self.request_item and self.request_item.school and not self.request_item.school.motif_require then
    return nil
  end
...
function Parser:ToDolRequest(unique_id)
  ...
  return {
    ["function"] = "CraftSmithingItemByLevel",
    ["args"] = args
  }
end
  ...
  for cat_i = 1, cat_ct do
    ...
    for subcat_i = 0, subcat_ct do
      ...
      for achieve_i = 1, achieve_ct do
        local achieve_id = GetAchievementId(cat_i, subcat_ii, achieve_i)
